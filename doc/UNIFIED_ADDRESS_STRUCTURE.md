...
公共代码位于本 `components/` 目录：

- `normalize_city.py`使用固定城市资产转换标准城市名称，并通过高德返回直接下级行政单位；其输出的`subdivisions`仅供上层场景使用，当前地址规范化和地图路由不会据此推断或补齐区县；
...
| `normalized_address` | `normalized_address` | 地址规范化 | 以目标城市为前缀，保留原地址中可识别行政区和具体位置的规范化地址 |
...
| 字段 | 含义 |
| --- | --- |
| `normalized_address` | 以目标城市为前缀，保留原地址中可识别行政区和具体位置的规范化地址；当前不根据`subdivisions`补齐缺失区县 |
...
| `complete` | 地址属于目标城市，并且包含可用的具体位置；普通地级市地址可缺少区县，直辖市地址缺少区县时为`partial` |
...
1. `original_address`非空且规范化完整：
   - 调用地图进行交叉验证；
   - 地图地址单独保存；
   - 地图返回的区县等行政信息仅保留在`map_address`中用于校验，不回填`normalized_address`或`final_address`；
