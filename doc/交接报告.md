...
三个 Skill 的骨架相同，差异集中在 `scripts/`：

```text
.agents/skills/
├── query-city-basic-education/
│   ├── SKILL.md · AGENTS.md · requirements.txt
│   ├── references/government-source-format.md
│   └── scripts/
│       ├── school_government_flow.py    政府资料流程 CLI
│       ├── school_common.py             字段词表、记录构造、校区拆分与去重
│       ├── review.py                    提取计划规则精确批准
│       ├── build_excel.py               行政单位与城市总表
│       ├── school_quality_check.py      交付质量闸门
│       └── tests/                       7 个测试文件
├── query-city-medical-institutions/
│   ├── SKILL.md · AGENTS.md · requirements.txt
│   ├── references/medical-source-format.md
│   └── scripts/
│       ├── medical_government_flow.py   政府资料流程 CLI
│       ├── medical_common.py            医疗记录构造、执业地址拆分与去重
│       ├── medical_scope.py             城市级去重、物理地址分桶、类型归并
│       ├── build_excel.py               区级工作簿与城市总表
│       ├── medical_quality_check.py     交付质量闸门
│       └── tests/                       5 个测试文件
└── query-city-universities/
    ├── SKILL.md · AGENTS.md · requirements.txt
    ├── assets/                          全国普通高校名单 / 985 / 211
    ├── references/retrieval-result-format.md
    └── scripts/
        ├── university_filter.py         按城市筛选高校与院校标签
        ├── university_domain.py         域名探测与批次汇总 CLI
        ├── university_page_fetch.py     单校页面抓取与校区/地址提取
        ├── university_fetch_runner.py   按域名清单分片并发抓取
        ├── university_results_merge.py  逐校结果合并与覆盖校验
        ├── build_university_address.py  页面结果 → 候选地址对 → 地址记录
        ├── university_campus_rules.py   校区语义与地址清洗规则
        ├── build_excel.py               高校信息与异常校工作簿
        ├── university_quality_check.py  交付质量闸门
        └── tests/                       8 个测试文件
```
...
| 场景 | 流程入口 | 子命令 |
| --- | --- | --- |
| 基础教育 | `school_government_flow.py` | `list-links` / `download` / `collect-details` / `inspect` / `preview` / `mineru-parse` / `mineru-inspect` / `extract` |
| 医疗机构 | `medical_government_flow.py` | `list-links` / `download` / `collect-details` / `platform-query` / `inspect` / `extract` |
| 高等教育 | `university_domain.py` | `probe` / `merge` |

两条跨 Skill 的约定：

- 三个质量闸门脚本（`*_quality_check.py`）接口统一为 `--input-dir <运行目录>`，报告缺省写 `<运行目录>/quality_report.json`，未通过时退出码非 0；公共脚手架取自 `query_city_core.quality_gate`。
- 三个 Skill 的 `scripts/` 存在同名模块（如各自的 `build_excel.py`），测试必须按 Skill 目录分别执行，同一进程内一起收集会互相覆盖。
