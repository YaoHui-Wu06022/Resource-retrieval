...
class FallbackAddressTests(unittest.TestCase):
    ...
    def test_other_administrative_unit_is_rejected(self):
        """基础教育兜底不得采用其他区县的同名学校。"""
        poi = build_poi('金星小学', '岐山大街4号', '白云区')
        candidate, status, _ = select_fallback_candidate(
            '金星小学',
            '广州市',
            [poi],
    ...
    def test_same_administrative_unit_is_accepted(self):
        """名称和目标区县均一致的唯一 POI 可以兜底。"""
        poi = build_poi('金星小学', '市桥街测试路1号', '番禺区')
        candidate, status, _ = select_fallback_candidate(
            '金星小学',
            '广州市',
            [poi],
        ...
        self.assertEqual(status, 'fallback')
        self.assertEqual(candidate['map_address'], '广州市番禺区市桥街测试路1号')
