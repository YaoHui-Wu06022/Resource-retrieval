...
- 命令示例相对 skill 根目录，实际按仓库 AGENTS.md 加 `conda run --no-capture-output -n py3.10 python -X utf8`。
- 缺依赖/密钥/公共组件即停下说明缺项；不自装依赖、不写一次性临时脚本复制固定逻辑。
- 输出到 `output/<标准城市名>/<YYYY-MM-DD>/Basic_Education/<HHMMSS>/`；每次调用新建 HHMMSS，不覆盖旧结果。

## 流程与分工

命令链：`city → download/collect-details(按需) → inspect → review.py → preview → extract → address.process → build_excel`。

| 步骤 | 命令 |
| --- | --- |
| 行政单位 | `python -m query_city_core.address.city --city <城市>` |
| 保存来源/详情页 | `scripts/school_government_flow.py download/list-links/collect-details`（详见 §2） |
| 生成提取计划 | `scripts/school_government_flow.py inspect --sources <government_source.json> --output <plan.json>` |
| 扫描件转录/页摘要 | `scripts/school_government_flow.py mineru-parse / mineru-inspect`（详见参考文档视觉节） |
| 批准规则 | `scripts/review.py <plan.json> <plan_config_<区>.json>` |
| 行覆盖复核 | `scripts/school_government_flow.py preview --plan <plan.json>` |
| 提取 | `scripts/school_government_flow.py extract --plan <plan.json> --output <address_records.json>` |
| 地址规范化 | `python -m query_city_core.address.process`（串行） |
| 工作簿 | `scripts/build_excel.py --input-dir <Basic_Education目录> --output <总表.xlsx>` |

Agent 负责来源检索、质量/覆盖判断与计划复核配置；脚本负责类型规范、去重、地址路由、排序和工作簿格式。按 `subdivisions` 顺序逐单位完成 §1–§4，再串行执行 §5–§6。
...
- `scripts/review.py <plan.json> <plan_config_<区>.json>` 按配置批准规则；配置结构：`district`、`approved_pdf_pages`（报考指南可选）、`files[{file, location_key, rules[{kind, location 或 pages("15-33"), name_cols, addr_col, fixed_type/fixed_nature/type_col/nature_col, required, fill_down, exclude_rows}]}]`；`pages` 自动展开，缺省表头按视觉 JSON 首页自动判定；报考指南只保留确实含本区单元格的页/表规则。
...
`extract` 输出 `address_records.json`；仅当 `stage=address_records` 且 `metrics.error_count=0` 才继续。记录须含学校名称、原始地址、学校类型、办学性质、发布日期、行政单位、可定位来源；`source_nature=government_information`；来源无地址留空，不推断、不编造。脚本按「校名+原始地址」合并并按学段去重，保留官方类型标签（如小学+九年一贯制→九年一贯制）。
...
`build_excel.py` 读各单位 `processed_address_records.json`，输出每区工作簿 + 城市总表。区级工作簿含「学校信息」（最终地址非空）与「异常校」（最终地址为空及异常原因）两表；去重、排序、列序等以脚本结果为准，Agent 不手工调整。
