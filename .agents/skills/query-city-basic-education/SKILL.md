...
| 检查来源、生成提取计划 | `scripts/build_school_address.py inspect` |
...
- 命令：`scripts/build_school_address.py list-links --input <manifest> --output <links.json>`
...
- 命令：`scripts/build_school_address.py download --manifest <manifest> --output-dir <行政单位目录>`
...
- 输出：本地文件 + 写回结果的清单；随后把 `local_files`、`local_file_urls`、`access_attempts` 写入 `government_source.json`

`collect-details`（名录页链接到同构详情页时）

- 输入：本地名录 HTML + 已复核的详情链接 CSS 选择器 + 允许域名
- 命令：`scripts/build_school_address.py collect-details --input <名录页.html> --link-selector <选择器> --output-dir <详情页目录> --manifest <详情页清单.json> --allowed-domain <域名>`
...
- 命令：`scripts/build_school_address.py inspect --input <government_source.json> --output <extraction_plan.json>`
...
PDF/图片标记 `needs_vision` 时按参考文档的视觉分支处理：有视觉能力则生成视觉结果并重跑检查；否则该来源按 `no_vision_capability` 跳过，不运行本地 OCR。

## 4. 提取学校记录

- 输入：已复核的 `extraction_plan.json`
- 命令：`scripts/build_school_address.py extract --plan <extraction_plan.json> --output <address_records.json>`
...
学校信息表列：序号、行政单位、学校名称、学校类型、办学性质、发布日期、地址、地址获取方式、地图匹配状态、信息来源。异常校表列：序号、行政单位、学校名称、学校类型、办学性质、异常原因、地图匹配状态、信息来源、发布日期。
