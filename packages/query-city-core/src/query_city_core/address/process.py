...
def resolve_address_record(
    ...
    return resolve_map(address_record, city_context, api_key, rate_limiter)
...
def resolve_address_payload(
    ...
    """规范化并按原顺序处理一批公共地址记录。"""
    normalized_payload = normalize_address_payload(input_payload)
    city_context = normalized_payload['city_context']
