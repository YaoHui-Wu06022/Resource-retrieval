...
def build_fallback_address(poi, city, city_prefixes):
    ...
    normalized = normalize_address_value(raw_address, city, city_prefixes)
    if normalized['normalization_status'] != 'complete':
        return ''
    return normalized['normalized_address']


def build_fallback_candidate(place_name, city, map_poi, city_prefixes):
    ...
    if normalize_place_name(place_name) != normalize_place_name(poi_name):
        return None
    if not is_target_city_poi(map_poi, city) or is_auxiliary_poi(map_poi):
        return None
    map_address = build_fallback_address(map_poi, city, city_prefixes)
    if not map_address:
        return None
    return {
...
def select_fallback_candidate(place_name, city, map_pois, city_prefixes):
    """按地址去重并仅在候选地址唯一时返回结果。"""
    map_candidates = {}
    for map_poi in map_pois:
        map_candidate = build_fallback_candidate(
            place_name, city, map_poi, city_prefixes
        )
        if map_candidate:
    ...
    if not map_candidates:
        return None, 'not_found', '没有符合名称、城市、类型和完整地址规则的 POI'
...
def resolve_fallback_map_address(
    ...
    map_candidate, map_status, map_reason = select_fallback_candidate(
        address_record['place_name'], city, map_pois, city_prefixes
    )
