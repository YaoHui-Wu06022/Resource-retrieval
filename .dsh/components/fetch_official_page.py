...
def collect_official_page_data(page, extra_address_labels=()):
    ...
    labels => {
      ...
      const stop = /\s*(?:(?:邮政编码|邮编|联系方式|查号台|传真|邮箱|电子邮箱|E[_-]?mail|版权所有|版权|Copyright|ICP备)\s*[：:]?|[\p{Script=Han}]{0,8}电话\s*[：:]|©)/iu;
      ...
      function region(e) { for(let n=e;n;n=n.parentElement){const r=String(n.getAttribute('role')||'').toLowerCase();if(n.tagName==='FOOTER'||r==='contentinfo')return 'semantic_footer';const h=hint(n);if(/(?:^|[ _-])(?:footer|foot|bottom|copyright)(?:$|[ _-])/.test(h))return 'footer_like';if(/(?:^|[ _-])(?:contact|address)(?:$|[ _-])/.test(h))return 'contact';}return 'body'; }
      function normLabel(v) { return clean(v).replace(/[\s：:]/g,''); }
      function isLabel(v) { const x=normLabel(v); return [...extras].some(label=>x.endsWith(label))||x==='地址'||(x.endsWith('地址')&&x.length<=14&&/^[\p{Script=Han}A-Za-z0-9（）()]+$/u.test(x)); }
      function inlines(v) { const found=[];for(const line of clean(v).split('\n')){const marks=[];for(let i=0;i<line.length;i++){if(!/[：:]/.test(line[i]))continue;const left=line.slice(0,i).trim(),spaced=left.match(/地\s*址$/),tail=left.split(/\s+/).at(-1)||'';const label=isLabel(tail)?tail:(spaced?spaced[0]:(isLabel(left)?left:''));if(label)marks.push({label,start:line.lastIndexOf(label,i),valueStart:i+1});}for(let i=0;i<marks.length;i++){const value=line.slice(marks[i].valueStart,marks[i+1]?.start??line.length).trim();if(value)found.push({label:marks[i].label,value,line});}}return found; }
      ...
      function looks(v, strength) { const raw=clean(v),x=cleanAddress(v);if(strength!=='strong'&&raw.includes('\n'))return false;return x.length>=4&&x.length<=160&&!bad.test(x)&&admin.test(x)&&(strength==='strong'||routePlace.test(x)||(place.test(x)&&/\d/.test(x))||x.split(admin).length>3); }
      ...
      for(const e of elements){const x=text(e);if(!x||x.length>500)continue;if(isLabel(x)){let v=related(e),value=v?text(v):adjacentText(e);if(!v&&value)v=e.parentElement;if(!v||region(e)==='body'&&(!visible(e)||!visible(v))&&!namedPanel(v))continue;const nestedElement=[...v.querySelectorAll('*')].find(c=>inline(text(c))&&![...c.children].some(k=>inline(text(k)))),nested=inline(text(nestedElement||v)),label=nested&&normLabel(nested.label)!=='地址'?nested.label:x;const leaves=[...v.querySelectorAll('*')].filter(c=>looks(text(c),'medium')&&![...c.children].some(k=>looks(text(k),'medium')));if(leaves.length<2)add(nested?nested.value:value,'label_relation','strong',nestedElement||v,`${x}\n${value}`,label);}}
      for(const e of elements){const x=text(e);if(!x||x.length>500)continue;if(region(e)==='body'&&((!visible(e)&&!namedPanel(e))||x.length>200))continue;const matches=inlines(x);if(!matches.length)continue;const child=[...e.children].some(c=>Boolean(inline(text(c))));if(!child){for(const m of matches)add(m.value,'inline_label','strong',e,x,m.label);if(region(e)!=='body'){let followsAddress=false;for(const line of clean(x).split('\n')){if(inline(line)){followsAddress=true;continue;}if(followsAddress&&line.split(admin).length>2)add(line,'inline_continuation','strong',e,line);}}}}
      const roots=elements.filter(e=>e.tagName==='FOOTER'||String(e.getAttribute('role')||'').toLowerCase()==='contentinfo'||/(?:^|[ _-])(?:footer|foot|bottom|copyright)(?:$|[ _-])/.test(hint(e))).filter((e,i,a)=>!a.some((r,j)=>j<i&&r.contains(e)));
      for(const root of roots){const named=root.querySelector('[class*="location" i],[id*="location" i],[class*="address" i],[id*="address" i]');if(!contact.test(text(root))&&!named)continue;const list=[root,...root.querySelectorAll('*')].filter(e=>!excluded.has(e.tagName)).sort((a,b)=>text(a).length-text(b).length);for(const e of list){const x=text(e);if(!x||isLabel(x)||inline(x)||/^[^：:\n]{1,12}[：:]/.test(x))continue;const child=[...e.children].some(c=>looks(text(c),'medium'));if(!child)add(x,'footer_contact','medium',e,x);}}
      const links=[...document.querySelectorAll('a[href]')].map(e=>({text:text(e).slice(0,120),url:e.href}));
      return {address_evidence:evidence,addressNodes:evidence,links};
    }
    """, labels)
