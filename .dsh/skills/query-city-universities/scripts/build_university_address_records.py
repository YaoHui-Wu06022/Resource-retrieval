...
PROCESSING_STATUSES = {'completed', 'skipped'}
...
def validate_base_information(base_payload):
    """校验基础信息并按学校标识码建立完整学校索引。"""
    if not isinstance(base_payload, dict) or base_payload.get('stage') != 'base_information':
        raise ValueError('同目录 base_information.json 阶段必须是 base_information')
    run = base_payload.get('run')
    city = str(run.get('city') or '').strip() if isinstance(run, dict) else ''
    if not city:
        raise ValueError('base_information.json 缺少标准城市名')
    schools = base_payload.get('schools')
    if not isinstance(schools, list):
        raise ValueError('base_information.schools 必须是数组')

    school_index = {}
    for school in schools:
        if not isinstance(school, dict) or set(school) != set(BASE_SCHOOL_FIELDS):
            raise ValueError('base_information 中的学校字段不完整或包含未知字段')
        ...
        if not school_identifier or not school_name:
            raise ValueError('基础学校必须包含学校标识码和学校名称')
        if school_identifier in school_index:
            raise ValueError(f'基础学校标识码重复：{school_identifier}')
        school_index[school_identifier] = school
    return city, school_index


def build_school_page_result(item, base_school):
    """用精简检索结果和基础信息组装单校页面结果。"""
    ...
    school_name = normalize_compact_text(base_school.get('school_name'))
    processing_status = str(item.get('processing_status') or '').strip()
    if processing_status not in PROCESSING_STATUSES:
        raise ValueError(
            f'{school_name}的 processing_status 必须是 completed 或 skipped'
        )
...
def build_page_results_payload(payload, base_payload):
    """补齐学校信息并构造完整高校页面结果批次。"""
    ...
    city, base_school_index = validate_base_information(base_payload)
    ...
    for item in items:
        ...
        if school_identifier in checked_school_identifiers:
            school_name = normalize_compact_text(
                base_school_index.get(school_identifier, {}).get('school_name')
            )
        ...
        page_results.append(build_school_page_result(
            item,
            base_school_index[school_identifier],
        ))

    missing_identifiers = set(base_school_index) - checked_school_identifiers
    if missing_identifiers:
        missing_names = [
            normalize_compact_text(base_school_index[identifier]['school_name'])
            for identifier in base_school_index
...
def collect_school_page_results(pages):
    ...
    for page in pages:
        ...
        for hint in page['campus_hints']:
            ...
            if not campus or campus in hint_keys:
                continue
    ...
    labeled_address_keys = {
        candidate['address_key'] for candidate in candidates if candidate['campus']
    }
    candidates = [
        candidate
        for candidate in candidates
        if candidate['campus'] or candidate['address_key'] not in labeled_address_keys
    ]
    addressed_campuses = {
        candidate['campus'] for candidate in candidates if candidate['campus']
    }
    hints = [hint for hint in hints if hint['campus'] not in addressed_campuses]
...
def build_address_payload(page_results):
    ...
    address_count = sum(bool(record['original_address']) for record in records)
    return {
        'schema_version': SCHEMA_VERSION,
        'stage': 'address_records',
        'city': page_results['city'],
        'items': records,
        'metrics': {
            'base_school_count': len(page_results['items']),
            ...
            'fallback_count': len(records) - address_count,
...
def build_output_payloads(retrieval_payload, base_payload):
    """一次构造页面批次归档和公共地址记录。"""
    page_results = build_page_results_payload(retrieval_payload, base_payload)
...
def main():
    ...
    try:
        ...
        base_path = input_path.parent / 'base_information.json'
